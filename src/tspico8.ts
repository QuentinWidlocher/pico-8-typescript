...
function init(workDir: string): void {
  ...
  existsSync(workDir) || mkdirSync(workDir);
  // Create the build directory if it doesn't exist
  existsSync(buildDir) || mkdirSync(buildDir);
  // Create an empty compiled.js file if it doesn't exist
  writeFile(compileDir, "", { flag: "wx" }, function (err) {
    if (err) throw err;
  });

  console.log(`The following files will be added to the ${workDir} directory:`);

  // Fetch all files to copy
  readdirSync(copyDir).forEach((file) => {
    console.log(file);
  });
  ...
  yesno({ question: "Proceed to copy? (y/n)" }).then((ok) => {
    ...
    readdirSync(copyDir).forEach((file: string) => {
...
function picoPath(workDir: string): string {
  ...
  if (
    process.platform in pico8PathMap &&
    existsSync(pico8PathMap[process.platform as keyof typeof pico8PathMap])
  ) {
    picoPath = pico8PathMap[process.platform as keyof typeof pico8PathMap];
  } else if (existsSync(cPico.executable)) {
...
function launchPico(
  ...
  let picoProcess: ChildProcessWithoutNullStreams | null = spawn(
    picoPath,
...
function compressGameFile(workDir: string): void {
  ...
  let buildStr = readFileSync(getOutfile(workDir), "utf8");
